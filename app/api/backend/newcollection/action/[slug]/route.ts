...
export async function GET(req) {
  try {
    ...
    const collection = await db.newCollection.findUnique({
      where: { id: Number(id) },
    });
...
export async function PUT(req) {
  try {
    ...
    if (!youtubeLink && !facebookLink) {
      return new Response(
        JSON.stringify({
          error: "Either YouTube or Facebook link is required",
        }),
        { status: 400 }
      );
    }
    if (youtubeLink && facebookLink) {
      return new Response(
        JSON.stringify({
          error: "Provide only one link at a time: either YouTube or Facebook",
        }),
        { status: 400 }
      );
    }
    ...
    if (
      (youtubeLink && !urlPattern.test(youtubeLink)) ||
      (facebookLink && !urlPattern.test(facebookLink))
    ) {
    ...
    const updatedCollection = await db.newCollection.update({
      where: { id: Number(id) },
      data,
    });
...
export async function DELETE(req) {
  try {
    ...
    const existingCollection = await db.newCollection.findUnique({
      where: { id: Number(id) },
    });
