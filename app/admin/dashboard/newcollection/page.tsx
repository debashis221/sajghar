"use client";
import { NewCollection } from "@prisma/client";
...
const NewCollectionTable = () => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(
          `/api/backend/newcollection/action/${id}`,
          {
            method: "DELETE",
          }
        );
  ...
  const filteredCollections = collections.filter((collection) => {
    const matchesType =
      typeFilter === "" ||
      (typeFilter === "youtube" && collection.youtube_link) ||
      (typeFilter === "facebook" && collection.facebook_link);
    const matchesFilter =
      (collection.youtube_link &&
        collection.youtube_link.toLowerCase().includes(filter.toLowerCase())) ||
      (collection.facebook_link &&
        collection.facebook_link.toLowerCase().includes(filter.toLowerCase()));
      ...
      <div className="card shadow-sm">
        <div className="card-header text-white">
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div>
              <Link
                href="/admin/dashboard/newcollection/create"
            ...
            <table className="table table-striped table-hover mb-4">
              ...
              <tbody>
                {[...Array(itemsPerPage)].map((_, index) => (
                  <tr key={index}>
                    <td>
                      <div className="skeleton w-50 h-20"></div>
                    </td>
                    <td>
                      <div className="skeleton w-150 h-50"></div>
                    </td>
                    <td>
            ...
            <table className="table table-striped table-hover mb-4">
              ...
              <tbody>
                {paginatedCollections.map((collection, index) => {
                  const videoLink =
                    collection.youtube_link || collection.facebook_link;
